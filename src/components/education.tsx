        ...
        <div className="flex flex-col items-start">
          ...
          <div className="align-baseline level-6 text-start opacity-60">
            {description.map((desc) => (
              <div key={desc} className="break-keep mb-0.5 level-6">
                {desc}
              </div>
            ))}
          </div>
...
export const Educations = () => {
  ...
  return (
    <div className={twMerge("flex flex-col gap-4", "transition-transform")}>
      <div className={twMerge("dark:text-white")}>{t("education")}</div>
      <div className="grid grid-cols-1">
        {educationList.map((educations) => (
          <div key={educations.school.name} className="grid grid-cols-8 gap-2">
            ...
            <div className="col-span-6 flex flex-col gap-4">
              <EducationCard {...(educations as Education)} />
            </div>
          </div>
        ))}
      </div>
